...
export function globalErrorHandler(error: unknown, _req: Request, res: Response, _next: NextFunction) {
  ...
  if (error instanceof ApplicationError) {
    ...
    console.error(`$NotificationService - globalErrorHandler() method error: `, prettyObject(error))
    return res.status(status).json({ message, error: errors })
  }

  // Unhandle Error
  console.error(`$NotificationService - globalErrorHandler() method unexpected error: `, prettyObject(error as {}))
